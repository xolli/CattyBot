...
public class ModeAddCommand(IServiceScopeFactory scopeFactory) : Command
{
    protected override async Task HandleCommand(ITelegramBotClient client, Message message, CancellationToken cancelToken)
    {
        if (message.From == null || message.Text == null || message.Chat == null) return;
        ...
        try
        {
            var prompt = service.CreateUserPrompt(name, content, message.Chat.Id);
            await client.SendMessage(
                message.Chat.Id,
                $"User prompt added: {prompt.Id} | {prompt.Name}\n*This prompt is only visible in this chat*",
